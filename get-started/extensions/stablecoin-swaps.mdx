---
title: "1:1 Stablecoin Settlement"
description: "USDT↔USDC conversion at a guaranteed 1:1 rate, no slippage, no market exposure."
---

## 1:1 Stablecoin Settlement

**Extension tier:** Premium

When the 1:1 Stablecoin Settlement extension is enabled, cross-stable conversions between USDT and USDC are executed at a guaranteed 1:1 rate. There is no slippage, no market exposure, and no variance between the deposited amount and the settled amount (net of the standard bridge fee).
...
Rhino.fi provides 1:1 support when the rate between USDT and USDC is up to 3bps (the Rhino.fi guardrail), it is then possible for client's who have the Advanced Fee and Limit Management extension to set an additional Client guardrail and continue providing a 1:1 experience for customers between the Rhino.fi and Client guardrail. Thereafter the activity will return to market rate.
