...
| <b>include</b> | Include chains or tokens | `include={...}` (URL-encoded JSON) |
...
### Including Chains or Tokens

Alternatively, you can control which chains and tokens are included in the widget by using the `include` option. This whitelists only the specified chains or tokens, hiding all others.

**Filter Option Type**
```ts
type ChainFilter = Record<string, boolean | string[]>
```

**How it works:**
- To **include an entire chain**, set its value to `true` or omit the token list.
- To **include specific tokens** for a chain, provide an array of token symbols to include.

**Examples:**

**Include a whole chain:**
```json
{
  "ETHEREUM": true
}
```
This will show only the Ethereum chain and hide all others.
...
```
This will show only USDT and USDC tokens on the Ethereum chain.
...
```
This includes USDT on Ethereum and all tokens on the Polygon chain.

> **Note:** When using `include`, only the specified chains and tokens will be visible in the widget.
